...
func emit_effects(hit_location: Vector2) -> void:
	hit_effect.emit(hit_location) 
...
func try_hit(hurtbox: Hurtbox, _is_chain_hit: bool = false) -> bool:
	if not is_new_hit(hurtbox.parent_stats):
		return false
	emit_effects(hurtbox.global_position)
	if hurtbox.is_blocking():
		hurtbox.parry_hit_effect.emit(global_position)
		return true
	# spawn particles, and/or send signal of successful attack
	update_hitlog(hurtbox.parent_stats)
	if not _is_chain_hit:
		landed_hit.emit(hurtbox.parent_stats)
	hurtbox.receive_hit(parent_stats, source_item, global_position.direction_to(hurtbox.global_position))
	return true
